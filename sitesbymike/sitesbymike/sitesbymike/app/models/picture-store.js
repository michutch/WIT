...
const pictureStore = {
  ...
  getAlbum(custid) {
    return this.store.findOneBy(this.collection, { custid: custid });
  },

  addPicture(custId, title, imageFile, response) {
    let album = this.getAlbum(custId);
    if (!album) {
      album = {
        custid: custId,
        photos: [],
